...
class BuySellStrategy:
    ...
    def on_bar(self, context: StrategyContext, data):
        ...
        if not self._entered:
            ...
            quantity = context.portfolio.position("AAPL").quantity  # type: ignore[union-attr]
            if quantity > 0:
                orders.append(Order(symbol="AAPL", quantity=-quantity))
        return orders
...
def test_backtest_engine_executes_strategy(tmp_path):
    ...
    config = BacktestConfig(symbols=["AAPL"], initial_cash=100_000.0, start=date(2024, 1, 1), end=date(2024, 1, 2))
...
def test_backtest_engine_applies_commission_and_slippage(tmp_path):
    ...
    config = BacktestConfig(
        ...
        commission_model=InteractiveBrokersCommission(per_share=0.01, minimum=1.0),
