#!/usr/bin/env python3
"""CLI utility to download Polygon daily bars into the local cache."""
...
def _parse_date(value: str) -> date:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid date format: {value}") from exc
...
def _resolve_universe_path(settings: AppSettings, name: str | None, override: Path | None) -> Path:
    ...
    if not name:
        raise ValueError(
            "Universe name or explicit file path must be provided.")
...
def _load_universe_symbols(path: Path, effective: date | None) -> list[str]:
    ...
    if effective is not None:
        ...
        raise ValueError(
            f"Effective date {effective} not found in universe {path}.")
    return sorted(latest_symbols(snapshots))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Polygon daily bars into Parquet cache.")
    parser.add_argument("symbols", nargs="*",
                        help="Ticker symbols to download (e.g., AAPL MSFT)")
    parser.add_argument(
        "--start", help="Start date (YYYY-MM-DD). Defaults to --end minus --years.")
    parser.add_argument(
        "--end", help="End date (YYYY-MM-DD). Defaults to today.")
    parser.add_argument(
        "--years",
        type=int,
        default=2,
        help="Lookback window in years when --start omitted (default: 2, limited by Polygon free tier).",
    )
    parser.add_argument(
        "--universe",
        help="Universe name under data/raw/universe to expand symbols (e.g., snp100).",
    )
    parser.add_argument("--universe-file", type=Path,
                        help="Explicit path to universe membership Parquet file.")
    parser.add_argument(
        "--effective-date",
        help="Filter universe membership to a specific effective date (YYYY-MM-DD). Defaults to latest.",
    )
    parser.add_argument("--dry-run", action="store_true",
                        help="Fetch data without writing Parquet files.")

    args = parser.parse_args()
    if not args.symbols and not args.universe and not args.universe_file:
        parser.error(
            "Provide ticker symbols or --universe/--universe-file to determine the download set.")
    return args


def resolve_symbols(args: argparse.Namespace, settings: AppSettings) -> list[str]:
    effective: date | None = _parse_date(
        args.effective_date) if args.effective_date else None
    if args.universe or args.universe_file:
        universe_path = _resolve_universe_path(
            settings, args.universe, args.universe_file)
...
def main() -> None:
    ...
    print(
        f"Processed {len(result.frames)} symbols from {start_date} to {end_date}.")
